...
const Certificado: React.FC = () => {
  ...
  if (error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-white bg-slate-900 p-6 text-center gap-4">
        ...
        <a
          href="/"
          className="px-8 py-3 bg-slate-700 text-white font-bold rounded-full hover:bg-slate-800 transition-all mt-2"
        >
          Volver al Inicio
        </a>
      </div>
  ...
  // Normaliza el nombre a "Title Case" sin importar cómo esté guardado
  const toTitleCase = (str: string) => str.toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase());

  // Usar nombre_curso del objeto relacional o del campo de texto (para MIS-)
  const courseName = cert?.curso?.nombre || cert?.nombre_curso || '_________________';
  const courseFontSize =
    courseName.length > 50 ? 'text-[15px]' : courseName.length > 35 ? 'text-[18px]' : 'text-[21px]';

  return (
    <div
      className="flex flex-col items-center justify-center p-4 min-h-screen print:p-0"
      ...
      <div className="w-full max-w-[900px] space-y-8 animate-in fade-in duration-700 print:space-y-0">
        <div className="text-center print:hidden">
          ...
          <p className="text-slate-300">
            Felicidades por este gran logro en tu carrera profesional.
          </p>
        </div>
        ...
        >
          ...
          >
            ...
            <div className="absolute inset-0 text-center" style={{ zIndex: 3 }}>
              ...
              >
                <h2
                  className="text-[42px] font-serif text-slate-700 font-bold"
                  style={{
                ...
                >
                  {toTitleCase(cert?.nombre_estudiante || 'Nombres y Apellidos')}
                </h2>
              </div>

              {/* Contenedor del Curso y Fecha */}
              <div
                className="absolute left-0 right-0 flex flex-col items-center justify-start text-slate-700"
                style={{ top: '350px', height: '110px', zIndex: 10 }}
              >
                {/* Texto Intermedio */}
                ...
                {/* Nombre del Curso */}
                <p
                  className={`font-serif font-bold mt-0.5 mb-1 text-slate-800 ${courseFontSize}`}
                  style={{
                ...
                <p
                  className="text-[13px] text-slate-500 font-serif"
                  style={{ margin: '2px 0 0 0' }}
                >
                  {cert?.fecha_inicio && cert?.fecha_fin ? (
                    <>
                      realizado del{' '}
                      {new Date(cert.fecha_inicio).toLocaleDateString('es-ES', {
                        day: 'numeric',
                        month: 'long',
                        year: 'numeric',
                      })}{' '}
                      al{' '}
                      {new Date(cert.fecha_fin).toLocaleDateString('es-ES', {
                        day: 'numeric',
                        month: 'long',
                        year: 'numeric',
                      })}
              ...
              >
                ...
                <div
                  style={{ borderTop: '1px solid #94a3b8', marginTop: '6px', width: '100%' }}
                ></div>
                <p className="text-[9px] font-sans text-slate-400 mt-1 mb-0 uppercase tracking-wider">
                  Fecha de Emisión
                </p>
              </div>
              ...
              <div
                className="absolute bottom-[48px] left-1/2 -translate-x-1/2 flex flex-col items-center"
              ...
              >
                {!sigFailed ? (
                  ...
                  <div
                    id="firma-fallback"
                    style={{
                      fontFamily: "'Great Vibes', 'Cedarville Cursive', 'Brush Script MT', cursive",
                      fontSize: '34px',
                      color: '#475569',
                ...
                <div className="flex flex-col items-center" style={{ width: '100%' }}>
                  <p className="text-[14px] font-sans font-medium text-slate-700 mb-0.5">
                    Docente Genérico
                  </p>
        ...
        <div className="flex flex-col items-center gap-4 pb-10 px-4 print:hidden">
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 w-full max-w-md sm:max-w-none">
            ...
            <a
              href="/"
              className="w-full sm:w-auto px-8 py-3 bg-slate-700 text-white font-bold rounded-full hover:bg-slate-800 transition-all text-center"
            >
              Volver al Inicio
            </a>
          </div>
