  ...
  useEffect(() => {
    ...
    const fetchLineaYDatos = async () => {
      try {
        ...
        const response = await apiClient.get(`/lineas-academicas/slug/${encodeURIComponent(slug)}`);

        const fullLineaData = response.data;
        ...
        const lineRutas = Array.isArray(fullLineaData.rutas_academicas)
          ? fullLineaData.rutas_academicas
          : [];
        ...
        const total = lineRutas.reduce((acc: number, ruta: any) => {
          return acc + (Array.isArray(ruta.cursos) ? ruta.cursos.length : 0);
        }, 0);
  ...
  return (
    <div className="min-h-screen bg-[#03070c] overflow-x-hidden text-white">
      ...
      <div className="relative pt-24 pb-20 px-6 md:px-10 overflow-hidden">
        <div
          className="absolute top-[-10%] right-[-5%] w-[600px] h-[600px] blur-[150px] rounded-full -z-10 animate-pulse opacity-80"
          style={{
            background: `radial-gradient(circle, rgba(14,165,233,0.12) 0%, transparent 70%)`,
          }}
        />
        <div
          className={`absolute top-[-10%] right-[-5%] w-[600px] h-[600px] ${accent.glow1} blur-[150px] rounded-full -z-10 animate-pulse`}
        />
        <div
          className={`absolute bottom-[-10%] left-[-5%] w-[500px] h-[500px] ${accent.glow2} blur-[120px] rounded-full -z-10`}
        />

        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-16 items-center relative z-10">
          {/* Text */}
          <div className="lg:w-1/2 w-full space-y-7 text-center lg:text-left">
            <nav
              aria-label="Miga de pan"
              className="inline-flex items-center gap-2.5 mb-8 md:mb-10 text-[10px] font-bold tracking-[0.2em] uppercase"
            >
              <Link to="/" className="text-slate-500 hover:text-sky-400 transition-colors">
                Inicio
              </Link>
              ...
              <span className="text-sky-400">{lineaData.nombre}</span>
            </nav>
            ...
            <div className="relative">
              <motion.div
                ...
                className="absolute -top-8 left-0 text-sky-400 font-black tracking-[0.8em] text-lg hidden lg:block"
              >
                MIS
              </motion.div>
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-10 pt-4">
              <div className="flex items-center gap-4">
                <div
                  className={`w-12 h-12 rounded-2xl border flex items-center justify-center ${accent.iconBg}`}
                >
                  <RouteIcon size={24} />
                </div>
                <div>
              ...
              <div className="flex items-center gap-4">
                <div
                  className={`w-12 h-12 rounded-2xl border flex items-center justify-center ${accent.iconBg2}`}
                >
                  <BookOpen size={24} />
                </div>
                <div>
          ...
          <div className="lg:w-1/2 w-full perspective-1000">
            ...
            >
              <div
                className={`absolute inset-0 ${accent.imgGlow} blur-[120px] rounded-full scale-150 animate-pulse -z-10`}
              />
              <motion.div
              ...
              >
                <div
                  className={`relative rounded-[3rem] overflow-hidden border border-white/10 ${accent.imgShadow}`}
                >
                ...
                <div
                  className={`absolute -top-4 -right-4 w-24 h-24 border-t-2 border-r-2 ${accent.corner} rounded-tr-[2rem] -z-10`}
                />
                <div
                  className={`absolute -bottom-4 -left-4 w-24 h-24 border-b-2 border-l-2 ${accent.corner} rounded-bl-[2rem] -z-10`}
                />
              </motion.div>
            </motion.div>
          </div>
      ...
      <div className="relative overflow-hidden">
        <div
          className={`absolute top-0 left-1/4 w-[600px] h-[500px] ${accent.sectionGlow1} rounded-full blur-[140px] pointer-events-none`}
        />
        <div
          className={`absolute bottom-0 right-1/4 w-[500px] h-[400px] ${accent.sectionGlow2} rounded-full blur-[130px] pointer-events-none`}
        />
        <LearningRoadmap
      ...
      <div className="relative max-w-7xl mx-auto px-6 md:px-10 py-24 overflow-hidden">
        <div
          className={`absolute top-0 right-0 w-[500px] h-[500px] ${accent.sectionGlow1} rounded-full blur-[140px] pointer-events-none`}
        />
        <div
          className={`absolute bottom-0 left-0 w-[400px] h-[400px] ${accent.sectionGlow2} rounded-full blur-[130px] pointer-events-none`}
        />

        <div className="relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
            <div className="space-y-4">
              <h2 className="text-3xl md:text-5xl font-black tracking-tight">Explora las Rutas</h2>
          ...
          {filteredRutas.length === 0 ? (
            ...
            <LineasAcademicasGrid
              lineas={filteredRutas}
              linea={lineaData}
              lineaNombre={lineaData.nombre}
            />
          )}
      ...
      <section className="relative py-24 px-6 text-center overflow-hidden">
        <div
          className={`absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[300px] ${accent.ctaGlow} rounded-full blur-[120px] pointer-events-none`}
        />
