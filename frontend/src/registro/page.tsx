        ...
        <div className="bg-black/60 backdrop-blur-xl border border-white/10 shadow-2xl rounded-[2rem] sm:rounded-[2.5rem] p-4 sm:p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-2.5">
            ...
            <div className="flex flex-col gap-1 pl-2 mt-2">
              <div
                className={`text-[9px] font-bold uppercase tracking-wider ${passLen >= 8 ? 'text-sky-400' : 'text-slate-400'}`}
              >
                {passLen >= 8 ? '✓ Mínimo 8 caracteres' : `⚠ Mínimo 8 caracteres (${passLen}/8)`}
              </div>
              <div
                className={`text-[9px] font-bold uppercase tracking-wider ${hasSpecialChar ? 'text-sky-400' : 'text-slate-400'}`}
              >
                {hasSpecialChar
            ...
            {success && (
              <motion.div
                initial={{ opacity: 0, y: -5 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-2xl text-emerald-400 text-xs font-bold text-center"
              >
                {success}
              </motion.div>
            )}
          ...
          >
            ...
            >
              ...
              <p className="text-xs text-white/70 font-medium leading-relaxed mb-8">
                Detectamos que solo ingresaste un apellido. Recuerda que para que tu certificado
                tenga plena **validez oficial** ante empresas e instituciones, es altamente
                recomendable registrar tus **dos apellidos** (Paterno y Materno).
              </p>
