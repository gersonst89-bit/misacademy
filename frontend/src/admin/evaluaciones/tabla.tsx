    ...
    return (
        <div ref={ref} className="relative">
            ...
            {open && (
              ...
              `}
                        >
                            <div className={`w-1.5 h-1.5 rounded-full ${e.value === 'Publicado' ? 'bg-emerald-500' :
...
export function Evaluaciones() {
    ...
    async function fetchCursos() {
        try {
            ...
            const d = res.data;
            let todos = [...(d.data || [])];
            const ultima = d.last_page || 1;

            if (ultima > 1) {
                const promesas = [];
                for (let pagina = 2; pagina <= ultima; pagina++) {
                    promesas.push(
                        apiClient.get("/admin/cursos", {
                            params: { page: pagina }
                        }).then(r => r.data)
                    );
                }
    ...
    async function fetchEvaluaciones() {
        ...
        try {
            ...
            if (cursoFiltro) {
                params.id_curso = cursoFiltro;
            }

            const res = await apiClient.get("/admin/evaluaciones", {
                params
            });
    ...
    const crear = async (payload: any) => {
        try {
            await apiClient.post("/admin/evaluaciones", payload);
    ...
    const guardar = async (ev: any) => {
        try {
            const { id_evaluacion, ...cleanEv } = ev;
            await apiClient.put(`/admin/evaluaciones/${id_evaluacion}`, cleanEv);
            await fetchEvaluaciones();
            return true;
        } catch (error) {
            console.error(error);
    ...
    const confirmarCambioEstado = async () => {
        ...
        try {
            const payload = {
                id_curso: evalEstadoSel.id_curso,
                titulo: evalEstadoSel.titulo,
                descripcion: evalEstadoSel.descripcion,
                duracion_minutos: evalEstadoSel.duracion_minutos,
                intentos_permitidos: evalEstadoSel.intentos_permitidos,
                puntaje_aprobatorio: evalEstadoSel.puntaje_aprobatorio ? Number(evalEstadoSel.puntaje_aprobatorio) : undefined,
                aleatorio: evalEstadoSel.aleatorio,
                tipo: evalEstadoSel.tipo,
                estado: nuevoEstado,
            };
            await apiClient.put(`/admin/evaluaciones/${evalEstadoSel.id_evaluacion}`, payload);
    ...
    const handleEliminar = async () => {
        ...
        try {
            await apiClient.delete(`/admin/evaluaciones/${evalToDelete.id_evaluacion}`);
