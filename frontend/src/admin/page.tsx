...
export default function AdminLayout() {
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      // No impedir el cierre local si falla la red
        ...
        return (
          <>
            ...
            >
              ...
              <div
                className={`px-6 mb-10 flex items-center justify-between relative z-10 ${isCollapsed ? 'justify-center px-0' : ''}`}
              >
                {showLabels && (
                  <Link to="/admin" className="flex items-center gap-3 group">
                    ...
                    <div className="flex flex-col">
                      <span className="font-black text-sm leading-tight tracking-tight text-white uppercase italic text-[11px] whitespace-nowrap">
                        MIS <span className="text-sky-500">Academy</span>
                      </span>
                      <span className="text-[7px] uppercase tracking-[0.4em] text-white/30 font-black">
                        Admin Panel
                      </span>
                    </div>
                  </Link>
              ...
              <nav className="flex-1 px-4 space-y-1 overflow-y-auto sidebar-scrollbar pb-8 relative z-10">
                {showLabels && (
                  <div className="text-[8px] uppercase tracking-[0.3em] text-white/20 font-black px-5 mb-4 mt-2">
                    Navegación
                  </div>
                )}
                ...
                >
                  ...
                  {showLabels && (
                    <div className="w-1.5 h-1.5 rounded-full bg-sky-500 shadow-[0_0_10px_rgba(14,165,233,1)] animate-pulse" />
                  )}
                </Link>
                ...
                {showLabels && (
                  <div className="pt-4 text-[7px] uppercase tracking-[0.3em] text-white/20 font-black px-4 mb-2">
                    Estructura
                  </div>
                )}
                <button
                  onClick={() => handleMenuClick('cursos')}
                  className={menuItemClass('cursos')}
                >
                  <BookOpen size={16} />
                  {showLabels && (
                    <span className="text-[10px] uppercase tracking-wider">Cursos</span>
                  )}
                </button>

                {!isBlockedForTeacher('rutas') && (
                  <button
                    onClick={() => handleMenuClick('rutas')}
                    className={menuItemClass('rutas')}
                  >
                    <RouteIcon size={16} />
                    {showLabels && (
                      <span className="text-[10px] uppercase tracking-wider">Rutas</span>
                    )}
                  </button>
                )}
                ...
                {!isBlockedForTeacher('lineas') && (
                  <button
                    onClick={() => handleMenuClick('lineas')}
                    className={menuItemClass('lineas')}
                  >
                    <Layers size={16} />
                    {showLabels && (
                ...
                {!isBlockedForTeacher('usuarios') && (
                  <button
                    ...
                    className={menuItemClass('usuarios')}
                  >
                    <Users size={16} />
                    {showLabels && (
                      <span className="text-[10px] uppercase tracking-wider">Usuarios</span>
                    )}
                  </button>
                )}
                ...
                {!isBlockedForTeacher('pagos') && (
                  <button
                    ...
                    className={menuItemClass('pagos')}
                  >
                    <CreditCard size={16} />
                    {showLabels && (
                      <span className="text-[10px] uppercase tracking-wider">Pagos</span>
                    )}
                  </button>
                )}
                ...
                {!isBlockedForTeacher('certificados') && (
                  <button
                    onClick={() => handleMenuClick('certificados')}
                    className={menuItemClass('certificados')}
                  >
                    <Award size={16} />
                    {showLabels && (
                      <span className="text-[10px] uppercase tracking-wider">Certificados</span>
                    )}
                  </button>
                )}
                ...
                {!isBlockedForTeacher('reclamaciones') && (
                  <button
                    onClick={() => handleMenuClick('reclamaciones')}
                    className={menuItemClass('reclamaciones')}
                  >
                    <BookOpen size={16} />
                    {showLabels && (
                      <span className="text-[10px] uppercase tracking-wider">Reclamaciones</span>
                    )}
                  </button>
                )}
                ...
                <button
                  onClick={() => handleMenuClick('modulos')}
                  className={menuItemClass('modulos')}
                >
                  <Layers size={16} />
                  {showLabels && (
                    <span className="text-[10px] uppercase tracking-wider">Módulos</span>
                  )}
                </button>
                <button
                  onClick={() => handleMenuClick('lecciones')}
                  className={menuItemClass('lecciones')}
                >
                  <Video size={16} />
                  {showLabels && (
                    <span className="text-[10px] uppercase tracking-wider">Lecciones</span>
                  )}
                </button>
                <button
                  onClick={() => handleMenuClick('evaluaciones')}
                  className={menuItemClass('evaluaciones')}
                >
                  <FileCheck size={16} />
                  {showLabels && (
                    <span className="text-[10px] uppercase tracking-wider">Exámenes</span>
                  )}
                </button>
                <button
                  onClick={() => handleMenuClick('materiales')}
                  className={menuItemClass('materiales')}
                >
                  <Search size={16} />
                  {showLabels && (
                    <span className="text-[10px] uppercase tracking-wider">Recursos</span>
                  )}
                </button>

                {showLabels && (
                  <div className="pt-8 text-[8px] uppercase tracking-[0.3em] text-white/20 font-black px-5 mb-4">
                    Ajustes del Sistema
                  </div>
                )}
                ...
                <button
                  ...
                  className={menuItemClass('notificaciones')}
                >
                  <Bell size={18} />
                  {showLabels && (
                    <span className="text-[11px] uppercase tracking-wider">Notificaciones</span>
                  )}
                </button>

                <button
                  onClick={() => handleMenuClick('configuracion')}
                  className={menuItemClass('configuracion')}
                >
                  <Settings size={18} />
                  {showLabels && (
                    <span className="text-[11px] uppercase tracking-wider">Configuración</span>
                  )}
                </button>
              </nav>

              <div
                className={`mt-auto pt-6 border-t border-white/5 relative z-10 bg-[#050a10]/80 backdrop-blur-xl ${isCollapsed ? 'px-0 flex flex-col items-center pb-6' : 'px-5 pb-6'}`}
              >
                <div
                  className={`group transition-all duration-500 cursor-pointer ${isCollapsed ? 'mb-6' : 'bg-white/5 rounded-2xl p-4 flex items-center gap-3 mb-4 border border-white/5 hover:bg-sky-500/10 hover:border-sky-500/20'}`}
                >
                  <div className="relative">
                    <div
                      className={`${isCollapsed ? 'w-12 h-12 rounded-xl' : 'w-11 h-11 rounded-xl'} overflow-hidden bg-gradient-to-br from-slate-800 to-black border border-white/10 flex items-center justify-center text-white font-black shadow-2xl group-hover:border-sky-500 transition-all text-[12px]`}
                    >
                      {(() => {
                        const imgPath =
                    ...
                    <div
                      className={`absolute -bottom-1 -right-1 bg-emerald-500 border-2 border-[#050a10] rounded-full shadow-[0_0_10px_rgba(16,185,129,1)] ${isCollapsed ? 'w-3.5 h-3.5' : 'w-3 h-3'}`}
                    />
                  </div>
                  {showLabels && (
