...
const uiToPublicado = (ui: EstadoUI) => (ui === 'Activo' ? 'Publicado' : 'Archivado');
...
function FiltroCurso({
  ...
  const labelActual =
    (value !== '' && cursos.find((c) => c.id_curso === value)?.nombre) || 'Todos los cursos';
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <div
          className={`w-1.5 h-1.5 rounded-full ${value !== '' ? 'bg-sky-500 animate-pulse' : 'bg-slate-300'}`}
        />
        <span className="truncate text-[10px] font-black uppercase tracking-widest text-slate-700 group-hover:text-sky-600 transition-colors max-w-[150px]">
          {labelActual}
        </span>
        <IoChevronDownOutline
      ...
      {open && (
        <div className="absolute top-full left-0 mt-3 w-80 bg-white rounded-[2rem] shadow-[0_30px_60px_rgba(15,23,42,0.15)] border border-slate-100 p-2 z-50 animate-fadeIn overflow-hidden backdrop-blur-xl bg-white/95">
          ...
          <div className="p-3 relative group">
            <IoSearchOutline
              className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-sky-500 transition-colors"
              size={14}
            />
            <input
              autoFocus
          ...
          <div className="max-h-72 overflow-y-auto p-1 custom-scrollbar">
            ...
            {opciones.map((c) => (
              ...
              >
                <div
                  className={`w-1.5 h-1.5 rounded-full ${value === c.id_curso ? 'bg-sky-500 shadow-[0_0_8px_rgba(14,165,233,0.5)]' : 'bg-slate-200'}`}
                />
...
function FiltroEstado({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <div
          className={`w-1.5 h-1.5 rounded-full ${value === 'Activo' ? 'bg-emerald-500 animate-pulse' : value === 'Inactivo' ? 'bg-rose-500' : 'bg-sky-500'}`}
        />
...
export function Modulos() {
  ...
  const fetchCursos = async () => {
    try {
      const res = await apiClient.get('/admin/cursos/menu');
      setCursos(res.data || []);
    } catch {
      setCursos([]);
    }
  ...
  const crear = async (
    ...
    try {
      ...
      console.error(e);
      const errMsg =
        e.response?.data?.message || e.message || 'Ocurrió un error al crear el módulo.';
    ...
    <div className="space-y-6 md:space-y-8 animate-fadeIn">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-4 bg-white p-4 md:p-3 rounded-[1.5rem] md:rounded-[2rem] shadow-sm border border-slate-100 transition-all duration-500">
        <div className="relative flex-1 group md:ml-2">
          <IoSearchOutline
            className="absolute left-0 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-sky-500 transition-colors duration-300"
            size={18}
          />
          <input
            type="text"
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
          <div className="w-full sm:w-auto">
            <FiltroCurso value={cursoFiltro} onChange={setCursoFiltro} cursos={cursos} />
          </div>
        ...
        {isLoading && (
          <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] z-10 flex items-center justify-center transition-all duration-500">
            <div className="flex flex-col items-center gap-4">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-sky-600 animate-pulse">
                Sincronizando...
              </span>
            </div>
          </div>
                ...
                <tr>
                  <td colSpan={5} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-4 grayscale opacity-50">
                      ...
                      <span className="text-slate-500 font-medium tracking-tight uppercase text-[10px] font-black tracking-widest">
                  ...
                  >
                    <td className="px-8 py-6">
                      <div className="flex flex-col">
                        <span className="text-[15px] font-black text-slate-900 group-hover:text-sky-600 transition-colors tracking-tight leading-tight">
                          {m.titulo}
                        </span>
                        <div className="flex items-center gap-2 mt-2.5">
                          <span className="text-[9px] bg-slate-100 text-slate-500 px-2.5 py-1 rounded-xl font-black uppercase tracking-widest border border-slate-200/50 shadow-sm">
                            ID: #{m.id_modulo}
                          </span>
                          <span className="text-[9px] bg-sky-50 text-sky-600/70 px-2.5 py-1 rounded-xl font-black uppercase tracking-widest border border-sky-100/50 shadow-sm">
                            Módulo Académico
                          </span>
                        </div>
                      </div>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col">
                        <span
                          className="text-[13px] font-bold text-slate-700 leading-snug max-w-[250px] truncate"
                          title={cursosById[(m as any)[FK]]}
                        >
                          {cursosById[(m as any)[FK]] || (m as any)[FK]}
                        </span>
                        <span className="text-[9px] text-sky-500 font-black uppercase tracking-widest mt-1">
                          Programa Académico
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-8 py-6">
                      <div
                        className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-[0.15em] border transition-all duration-500
                        ${
                      ...
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${(m.estado as EstadoUI) === 'Activo' ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500 animate-pulse'}`}
                        />
                        {m.estado}
                      </div>
                    </td>
        ...
        <div className="md:hidden divide-y divide-slate-100">
          {filtrados.length === 0 && !isLoading ? (
            <div className="p-10 text-center text-slate-400 font-medium italic">
              No se encontraron módulos académicos
            </div>
          ) : (
            filtrados.map((m) => (
              <div
                key={m.id_modulo}
                className="p-5 flex flex-col gap-4 bg-white hover:bg-slate-50 transition-colors duration-300"
              >
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <span className="text-[14px] font-black text-slate-900 leading-tight tracking-tight block truncate group-hover:text-sky-600 transition-colors">
                      {m.titulo}
                    </span>
                    <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase tracking-widest truncate">
                      {cursosById[(m as any)[FK]] || 'Sin curso vinculado'}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-[8px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-md font-black uppercase tracking-widest border border-slate-200/50">
                      ID: #{m.id_modulo}
                    </span>
                    <div
                      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest border transition-all
                    ...
                    >
                      <div
                        className={`w-1 h-1 rounded-full ${(m.estado as EstadoUI) === 'Activo' ? 'bg-emerald-500' : 'bg-rose-500'}`}
                      />
                      {m.estado}
                    </div>
                  </div>
                  <div className="flex items-center gap-1 bg-white p-1 rounded-xl shadow-sm border border-slate-100">
                    <button
                      ...
                      className="p-2.5 text-amber-500 hover:bg-amber-50 rounded-lg transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="p-2.5 text-sky-500 hover:bg-sky-50 rounded-lg transition-colors"
                    >
        ...
        <div className="px-6 md:px-8 py-5 bg-slate-50/50 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
              Página <span className="text-slate-900">{pagina}</span> de{' '}
              <span className="text-slate-900">{totalPaginas}</span>
            </span>
          </div>
      ...
      <InfoModuloModal
        ...
        cursoNombre={
          moduloInfo
            ? cursosById[(moduloInfo as any)[FK]] || String((moduloInfo as any)[FK])
            : undefined
        }
      />
