...
import { FaEdit, FaInfoCircle, FaPlus, FaSearch, FaArchive, FaChevronDown } from 'react-icons/fa';
...
export function RutasAcademicas() {
  ...
  const fetchRutas = async () => {
    ...
    setCargando(true);
    try {
      const response = await apiClient.get('/rutas-academicas/admin-list', {
        params: {
          page: pagina,
          per_page: 10,
  ...
  const handleConfirmCambioEstado = async () => {
    ...
    try {
      ...
      const response = await apiClient.put(`/rutas-academicas/${rutaEstado.id_ruta}`, cleanData);

      const data = response.data;

      setRutas((prev) =>
        prev.map((r) => (r.id_ruta === rutaEstado.id_ruta ? { ...r, estado: nuevoEstado } : r)),
      );
  ...
  const handleEliminar = async () => {
    ...
    try {
      ...
      alert(
        'Hubo un error al eliminar la ruta: ' +
  ...
  const handleGuardarEdicion = async (rutaActualizada: RutaAcademica): Promise<boolean> => {
    try {
      const cleanData = {
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fadeIn px-2 md:px-0">
      ...
      <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                ...
                <th className="px-8 py-5 text-[11px] font-black uppercase tracking-[0.25em] text-slate-600 text-right pr-12">
                  Inversión
                </th>
                <th className="px-8 py-5 text-[11px] font-black uppercase tracking-[0.25em] text-slate-600">
                  Estado
                ...
                [...rutasFiltradas]
                  ...
                  .map((ruta) => (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-6">
                          <div className="relative w-32 h-20 rounded-2xl overflow-hidden shadow-sm group-hover:shadow-md transition-all duration-500 border border-white shrink-0 bg-slate-100">
                            {ruta.imagen ? (
                              ...
                              <div className="w-full h-full flex items-center justify-center text-[10px] text-slate-400 uppercase font-black">
                                No img
                              </div>
                            )}
                          </div>
                          <div className="flex flex-col min-w-0">
                            <span className="text-[15px] font-extrabold text-slate-900 group-hover:text-sky-600 transition-colors tracking-tight leading-tight truncate">
                              {ruta.nombre}
                            </span>
                            <div className="flex items-center gap-2 mt-1.5">
                              <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest bg-white/80 px-2 py-0.5 rounded-md border border-slate-100 shadow-sm">
                                ID: #{ruta.id_ruta}
                              </span>
                              <span className="text-[9px] text-sky-500 font-black uppercase tracking-widest">
                                Master Route
                              </span>
                            </div>
                          </div>
                      ...
                      <td className="px-8 py-6">
                        <span
                          className={`inline-flex px-4 py-2 rounded-2xl text-[9px] font-black uppercase tracking-widest border backdrop-blur-md transition-all duration-300 ${
                            ruta.nivel?.toLowerCase() === 'avanzado'
                              ? 'border-amber-200/50 bg-amber-500/10 text-amber-600'
                              : ruta.nivel?.toLowerCase() === 'intermedio'
                                ? 'border-sky-200/50 bg-sky-500/10 text-sky-600'
                      ...
                      <td className="px-8 py-6">
                        <div
                          className={`inline-flex items-center gap-2.5 px-4 py-2 rounded-2xl text-[9px] font-black uppercase tracking-widest border backdrop-blur-md transition-all duration-500
                          ${
                        ...
                        >
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${ruta.estado === 'Publicado' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`}
                          />
                          {ruta.estado}
                        </div>
                      </td>
        ...
        <div className="md:hidden divide-y divide-slate-50">
          {cargando ? (
            ...
            <div className="p-16 text-center text-slate-400 font-bold italic text-xs">
            ...
            rutasFiltradas.map((ruta) => (
              <div
                key={ruta.id_ruta}
                className="p-6 space-y-5 bg-white hover:bg-slate-50/50 transition-colors"
              >
                <div className="flex items-start gap-4">
                  <div className="relative w-28 h-20 rounded-[1.5rem] overflow-hidden shadow-sm border border-slate-50 flex-shrink-0 bg-slate-100">
                    {ruta.imagen ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-[10px] text-slate-400 font-black uppercase">
                        No img
                      </div>
                    )}
                  </div>
                  <div className="flex flex-col flex-1 min-w-0">
                    <span className="text-base font-black text-slate-900 tracking-tight leading-tight mb-2 truncate">
                      {ruta.nombre}
                    </span>
                ...
                <div className="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl border border-slate-100">
                  <div className="flex flex-col pl-1">
                    ...
                    <span className="text-base font-black text-slate-900">
                      S/.{' '}
                      {Number(ruta.precio || 0).toLocaleString('es-PE', {
                  ...
                  <div className="grid grid-cols-4 gap-1.5 bg-white p-1 rounded-xl shadow-sm border border-slate-100">
                    <button
                      onClick={() => handleEditar(ruta)}
                      className="p-2.5 text-amber-500 hover:scale-95 transition-transform"
                    >
                      <FaEdit size={16} />
                    </button>
                    <button
                      onClick={() => handleEstadoClick(ruta)}
                      className="p-2.5 text-slate-400 hover:scale-95 transition-transform"
                    >
                      <FaArchive size={16} />
                    </button>
                    <button
                      ...
                      className="p-2.5 text-rose-500 hover:scale-95 transition-transform"
                    >
                      <svg
                        stroke="currentColor"
                        fill="none"
                    ...
                    <button
                      ...
                      className="p-2.5 text-sky-500 hover:scale-95 transition-transform"
                    >
                      <FaInfoCircle size={16} />
                    </button>
                  </div>
        ...
        <div className="px-6 md:px-8 py-5 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-5">
          <div className="flex flex-col md:flex-row items-center gap-3 text-center md:text-left">
            ...
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
              Resultados filtrados: {rutasFiltradas.length}
            </span>
          </div>
      ...
      <ArchiveModal
        ...
        nuevoEstado={
          (rutaEstado?.estado === 'Activa' ? 'Archivado' : 'Publicado') as 'Publicado' | 'Archivado'
        }
      />
      <DeleteModal
