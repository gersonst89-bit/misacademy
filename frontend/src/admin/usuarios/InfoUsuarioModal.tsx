      ...
      <div className="space-y-6 md:space-y-8">
        <div className="flex flex-col md:flex-row items-center gap-6 p-2 text-center md:text-left">
          <div className="w-24 h-24 md:w-28 md:h-28 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center text-slate-400 text-3xl md:text-4xl font-black shadow-2xl border-4 border-white relative group">
            {usuario.imagen_perfil ? (
              <img
                src={
                  /^https?:\/\//i.test(usuario.imagen_perfil)
                    ? usuario.imagen_perfil
                    : `${API_URL?.replace(/\/$/, '')}/${usuario.imagen_perfil.replace(/^\/?(api\/)?/, '')}`
                }
                ...
                onError={(e) => {
                  ...
                  if (parent) {
                    parent.innerText = usuario.nombre?.charAt(0)?.toUpperCase() || 'U';
                  }
                }}
              />
            ) : (
              <span className="opacity-50">{usuario.nombre?.charAt(0)?.toUpperCase() || 'U'}</span>
            )}
          ...
          <div className="flex flex-col items-center md:items-start">
            ...
            <div className="flex items-center gap-2 mt-3">
              <div
                className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider border backdrop-blur-md
                ${
                  usuario.id_rol === 1
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50/40 border border-slate-100/50 rounded-[2rem] md:rounded-[2.5rem] p-4 md:p-6">
          <div className="flex items-center gap-4 bg-white p-4 rounded-[1.5rem] border border-slate-100 shadow-sm">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[13px] font-bold text-slate-700 truncate block">
                {usuario.email}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 bg-white p-4 rounded-[1.5rem] border border-slate-100 shadow-sm">
            <div className="w-10 h-10 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center flex-shrink-0">
              ...
              >
                <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.81 12.81 0 0 0 .82 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.82A2 2 0 0 1 22 16.92z"></path>
              </svg>
            </div>
            <div className="flex flex-col">
              <span className="text-[9px] font-black uppercase tracking-widest text-slate-400">
          ...
          <div className="flex items-center gap-4 bg-white p-4 rounded-[1.5rem] border border-slate-100 shadow-sm">
            ...
            <div className="flex flex-col">
              ...
              <span
                className={`text-[13px] font-black ${usuario.estado?.toLowerCase() === 'activo' ? 'text-emerald-600' : 'text-rose-600'}`}
              >
                {usuario.estado}
              </span>
            </div>
        ...
        {usuario.biografia && (
          <div className="px-5 md:px-6 py-4 bg-slate-50/50 rounded-2xl border border-slate-100 mx-2 md:mx-0">
            <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 text-center md:text-left block">
              Biografía Profesional
            </span>
            <p className="text-[12px] md:text-[13px] text-slate-600 leading-relaxed mt-2.5 italic text-center md:text-left">
              "{usuario.biografia}"
            </p>
          </div>
        )}
