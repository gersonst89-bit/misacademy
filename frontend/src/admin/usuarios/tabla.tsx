...
function FiltroEstado({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <div
          className={`w-1.5 h-1.5 rounded-full ${value === 'Activo' ? 'bg-emerald-500 animate-pulse' : value === 'Inactivo' ? 'bg-slate-400' : 'bg-sky-500'}`}
        />
...
function FiltroRol({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
...
export default function UsuariosPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(`Usuario ${selectedUsuario.id_usuario ? 'actualizado' : 'creado'} con éxito`);
      setShowModal(false);
      fetchUsuarios();
    } catch (error: any) {
      console.error(error);
  ...
  const handleAccion = async () => {
    ...
    try {
      ...
      alert(
        err.message ||
          `Error al ${tipoAccion === 'desactivar' ? 'desactivar' : 'eliminar'} usuario`,
      );
    }
  };
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-fadeIn px-2 md:px-0">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 md:gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto">
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <FiltroRol value={filtroRol} onChange={setFiltroRol} />
          </div>
              ...
              {loading ? (
                ...
                usuarios.map((u) => (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl overflow-hidden bg-slate-100 flex items-center justify-center text-slate-600 font-black shadow-sm group-hover:scale-105 transition-all duration-500 border border-white shrink-0">
                          {u.imagen_perfil ? (
                            <img
                              src={
                                /^https?:\/\//i.test(u.imagen_perfil)
                                  ? u.imagen_perfil
                                  : apiUrl(u.imagen_perfil.replace(/^\/?(api\/)?/, ''))
                              }
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="text-[14px] font-extrabold text-slate-900 group-hover:text-sky-600 transition-colors tracking-tight leading-tight truncate">
                            {u.nombre} {u.apellido}
                          </span>
                    ...
                    <td className="px-8 py-6">
                      <div
                        className={`inline-flex items-center gap-2.5 px-4 py-2 rounded-2xl text-[9px] font-black uppercase tracking-widest border
                        ${
                          u.id_rol === 1
                    ...
                    <td className="px-8 py-6 text-center">
                      <div
                        className={`inline-flex items-center gap-2.5 px-4 py-2 rounded-2xl text-[9px] font-black uppercase tracking-widest border transition-all duration-500
                          ${
                      ...
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${u.estado?.toLowerCase() === 'activo' ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`}
                        />
                        {u.estado}
                      </div>
                    </td>
              ...
              >
                <div className="flex items-center gap-4">
                      ...
                      <img
                        ...
                        onError={(e) => {
                          ...
                          if (target.parentElement) {
                            target.parentElement.innerText = u.nombre?.charAt(0) || 'U';
                          }
                        }}
                      />
                    ) : (
                  ...
                  <div className="flex flex-col flex-1 min-w-0">
                    <span className="text-base font-black text-slate-900 tracking-tight leading-tight truncate">
                      {u.nombre} {u.apellido}
                    </span>
                ...
                <div className="grid grid-cols-4 gap-2 bg-slate-50 p-1.5 rounded-2xl border border-slate-100 shadow-sm">
                  <button
                    onClick={() => openModal(u)}
                    className="p-3 bg-white text-amber-500 rounded-xl shadow-sm flex items-center justify-center hover:scale-95 transition-transform"
                  >
                    <FaEdit size={18} />
                  </button>
                  <button
                    ...
                    className="p-3 bg-white text-slate-500 rounded-xl shadow-sm flex items-center justify-center hover:scale-95 transition-transform"
                    title="Desactivar"
                  >
                    <FaUserSlash size={18} />
                  </button>
                  <button
                    ...
                    className="p-3 bg-white text-rose-500 rounded-xl shadow-sm flex items-center justify-center hover:scale-95 transition-transform"
                    title="Eliminar Definitivamente"
                  >
                    <svg
                      stroke="currentColor"
                  ...
                  <button
                    ...
                    className="p-3 bg-white text-sky-500 rounded-xl shadow-sm flex items-center justify-center hover:scale-95 transition-transform"
                  >
        ...
        <div className="px-6 md:px-8 py-5 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-5">
          <div className="flex flex-col md:flex-row items-center gap-3 text-center md:text-left">
            ...
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
              Resultados filtrados: {usuarios.length}
            </span>
          </div>
      ...
      {showInfoModal && selectedUsuario && (
        <InfoUsuarioModal usuario={selectedUsuario} onClose={() => setShowInfoModal(false)} />
      )}
              ...
              {errorCompras.pagos && errorCompras.pagos.length > 0 && (
                <div className="space-y-3 mt-4">
                  ...
                  {errorCompras.pagos.map((pago: any) => (
                    <div
                      key={pago.id_pago}
                      className="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between md:items-center gap-3"
                    >
                      <div>
                        <p className="text-sm font-black text-slate-700">
                          Monto Total: S/ {pago.monto_total}
                        </p>
                  ...
                  <div className="mt-6 p-4 bg-amber-50 rounded-2xl border border-amber-100">
                    <p className="text-xs text-amber-700 font-bold leading-relaxed">
                      Recomendación: Si deseas ocultar a este usuario del sistema, por favor utiliza
                      la opción{' '}
                      <span className="font-black bg-amber-200/50 px-2 py-0.5 rounded">
                        "Desactivar"
                      </span>{' '}
                      en lugar de "Eliminar Definitivamente".
                    </p>
                  </div>
                </div>
              )}
