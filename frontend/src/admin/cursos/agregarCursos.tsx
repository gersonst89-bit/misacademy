...
import { IoBookOutline, IoTimeOutline, IoLayersOutline, IoSparklesOutline } from 'react-icons/io5';
...
export const AddCursoModal: React.FC<AddCursoModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const [estado, setEstado] = useState<'Publicado' | 'Activo' | 'Inactivo' | 'Archivado'>(
    'Archivado',
  );
    ...
    <AdminModal
      ...
      footer={
        <>
          ...
          <button
            onClick={onClose}
            className="w-full md:w-auto px-6 py-3 rounded-2xl font-bold text-gray-400 hover:bg-gray-100 transition-all text-sm"
          >
            Cancelar
          </button>
        </>
        ...
        <div className="pt-8 border-t border-gray-100 flex flex-wrap items-center justify-between gap-8">
          <div className="flex items-center gap-8">
            <SelectComponent
              ...
              value={estado}
              onChange={(e) =>
                setEstado(e.target.value as 'Publicado' | 'Activo' | 'Inactivo' | 'Archivado')
              }
              options={[
              ...
              <div className="relative">
                ...
                <div
                  className={`w-12 h-6 rounded-full transition-colors ${
                    destacado ? 'bg-sky-500' : 'bg-gray-200'
                  }`}
                />

                <div
                  className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-transform ${
                    destacado ? 'translate-x-6' : ''
                  }`}
                />
              </div>
