...
import { IoInformationCircleOutline, IoPersonOutline, IoStatsChartOutline } from 'react-icons/io5';
...
export function InfoCursoModal({ isOpen, onClose, curso }: Props) {
  const [nombreRuta, setNombreRuta] = useState<string>('Curso libre / Sin ruta');

  useEffect(() => {
    if (!curso) {
      setNombreRuta('Curso libre / Sin ruta');
      return;
    }
    ...
    if (typeof primeraRuta === 'object' && primeraRuta !== null && 'nombre' in primeraRuta) {
      setNombreRuta(
        typeof primeraRuta.nombre === 'string' && primeraRuta.nombre.trim()
          ? primeraRuta.nombre
          : 'Ruta asignada',
      );
      return;
    ...
    // Si por alguna razón la ruta llega solo como ID,
    // no inventamos un nombre.
    setNombreRuta('Ruta asignada');
  }, [curso]);
      ...
      <div className="space-y-6 md:space-y-8">
        ...
        <div className="relative h-48 md:h-64 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl group border border-white/10">
              ...
              src={
                typeof curso.imagen === 'string' && curso.imagen.startsWith('http')
                  ...
                  : typeof curso.imagen === 'string'
                    ? `${API_URL}/${
                        curso.imagen.startsWith('/') ? curso.imagen.slice(1) : curso.imagen
                      }`
                    : curso.imagen
              }
              alt={curso.nombre}
          ...
          <div className="absolute bottom-5 md:bottom-8 left-5 md:left-8 right-5 md:right-8">
            ...
            <h3 className="text-xl md:text-3xl font-black text-white leading-tight tracking-tight drop-shadow-lg">
              {curso.nombre}
            </h3>
          </div>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-2">
                <div className="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
                  ...
                  <p className="text-sm font-black text-slate-900 mt-0.5">
                    {curso.duracion_horas || curso.duracion || '—'} horas
                  </p>
                </div>
              ...
              <div className="pt-2">
                ...
                <p
                  className={`text-sm font-black mt-0.5 ${
                    nombreRuta === 'Curso libre / Sin ruta' ? 'text-slate-500' : 'text-sky-600'
                  }`}
                >
                  {nombreRuta}
                </p>
            ...
            <div className="bg-white rounded-[2rem] p-6 space-y-5 border border-slate-100 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <span className="text-2xl font-black text-slate-900 tracking-tighter">
                  {curso.precio && Number(curso.precio) > 0 ? `S/ ${curso.precio}` : 'Gratis'}
                </span>
              </div>
