...
function FiltroEstado({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <div
          className={`w-1.5 h-1.5 rounded-full ${value === 'Publicado' ? 'bg-emerald-500 animate-pulse' : value === 'Archivado' ? 'bg-amber-500' : 'bg-sky-500'}`}
        />
...
export function Cursos() {
  ...
  const [filtroEstado, setFiltroEstado] = useState<'' | 'Publicado' | 'Archivado'>('');
  ...
  const fetchRutas = async () => {
    try {
      const res = await apiClient.get('/rutas-academicas/menu');
      setRutas(res.data || []);
    } catch (error) {
  ...
  const crearCurso = async (
    ...
    try {
      ...
      const cleanData = {
        ...
        id_docente:
          (newCurso as any).id_docente === null ||
          (newCurso as any).id_docente === undefined ||
          (newCurso as any).id_docente === ''
            ? null
            : Number((newCurso as any).id_docente),
        rutas: newCurso.rutas.map((r) => (typeof r === 'number' ? r : (r as any).id_ruta)),
      };
      ...
      // El backend devuelve el curso directamente (con id_curso), no envuelto en { curso: ... }
      if (data && data.id_curso) {
        showToast('✅ Curso creado correctamente', 'success');
        // Refrescar la lista completa para que quede bien paginado
        fetchCursos();
        return true;
      }
      ...
      const errMsg =
        ...
        'Error al crear curso. Revisa consola.';
      alert(errMsg);
      return false;
    }
  };
  ...
  const guardarEdicion = async (cursoActualizado: Curso): Promise<boolean> => {
    try {
      const { id_curso, fecha_creacion, fecha_actualizacion, docente, slug, modulos, ...rest } =
      ...
      const cleanData = {
        ...
        duracion_horas:
          cursoActualizado.duracion_horas !== undefined && cursoActualizado.duracion_horas !== null
            ...
            : cursoActualizado.duracion !== undefined && cursoActualizado.duracion !== null
              ...
              : undefined,
        tiempo:
          cursoActualizado.tiempo !== undefined && cursoActualizado.tiempo !== null
        ...
        id_docente:
          cursoActualizado.id_docente === null || cursoActualizado.id_docente === undefined
            ? null
      ...
      if (cursoActualizadoRes && cursoActualizadoRes.id_curso) {
        // El PUT no devuelve las relaciones completas (rutas/docente),
        // así que las preservamos desde los datos que acabamos de enviar.

        const rutasActualizadas = cursoActualizado.rutas
          ?.map((r: any) => {
            const idRuta = typeof r === 'number' ? r : r?.id_ruta;

            return rutas.find((ruta) => ruta.id_ruta === idRuta) || r;
          })
          .filter(Boolean);
        ...
        const cursoFinal = {
          ...
          docente:
            cursoActualizado.id_docente === null || cursoActualizado.id_docente === undefined
              ? null
              : cursoActualizadoRes.docente ||
                cursoActualizado.docente ||
                cursoAEditar?.docente ||
                null,
          id_docente:
            cursoActualizado.id_docente === null || cursoActualizado.id_docente === undefined
              ? null
        ...
        setCursosFiltrados((prev) =>
          prev.map((c) => (c.id_curso === cursoFinal.id_curso ? cursoFinal : c)),
        );
      }
  ...
  const handleConfirmCambioEstado = async () => {
    ...
    const nuevoEstado = cursoEstadoSeleccionado.estado === 'Publicado' ? 'Archivado' : 'Publicado';

    try {
      await apiClient.patch(`/cursos/${cursoEstadoSeleccionado.id_curso}/estado`, {
        estado: nuevoEstado,
      });
  ...
  const handleEliminar = async () => {
    ...
    try {
      ...
      setCursosFiltrados((prev) => prev.filter((c) => c.id_curso !== cursoAEliminar.id_curso));
  ...
  const abrirModalEditar = async (curso: Curso) => {
    // Primero abrir el modal con los datos básicos del listado
    setCursoAEditar(curso);
    setIsEditModalOpen(true);
    setIsLoadingEdit(true);

    // Luego cargar los datos completos del curso (con todos los campos)
    try {
      const res = await apiClient.get(`/admin/cursos/${curso.id_curso}`);
      if (res.data && res.data.id_curso) {
        setCursoAEditar(res.data);
      }
    } catch (error) {
      console.error('Error cargando datos completos del curso:', error);
      // Si falla, se mantienen los datos del listado (sin los campos detallados)
    } finally {
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
              ...
              {isLoading ? (
                ...
                cursos.map((curso) => (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-6">
                        <div className="relative w-32 h-20 flex-shrink-0 rounded-[1.25rem] overflow-hidden shadow-sm group-hover:shadow-xl group-hover:-translate-y-1 transition-all duration-500 bg-slate-100">
                          {curso.imagen ? (
                            ...
                            <div className="w-full h-full bg-slate-100 flex items-center justify-center text-[10px] text-slate-400 uppercase font-black">
                              No img
                            </div>
                          )}
                    ...
                    <td className="px-8 py-7">
                      <span
                        className={`inline-flex px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border backdrop-blur-md transition-all duration-300 ${
                          curso.nivel?.toLowerCase() === 'avanzado'
                            ? 'border-amber-200/50 bg-amber-500/10 text-amber-600 shadow-[0_0_15px_rgba(245,158,11,0.1)]'
                            : curso.nivel?.toLowerCase() === 'intermedio'
                              ? 'border-sky-200/50 bg-sky-500/10 text-sky-600 shadow-[0_0_15px_rgba(14,165,233,0.1)]'
                    ...
                    <td className="px-8 py-7">
                      <div className="flex justify-center">
                        ...
                        >
                          <div
                            className={`w-2 h-2 rounded-full animate-pulse ${curso.estado === 'Publicado' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.8)]' : 'bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.8)]'}`}
                          />
                          {curso.estado}
                        </div>
                      </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4 pt-2">
                  <div className="flex flex-col">
                    ...
                    <span
                      className={`inline-flex px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border w-fit ${
                        curso.nivel?.toLowerCase() === 'avanzado'
                ...
                <div className="flex items-center justify-between pt-2">
                  <div
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase border ${
                      curso.estado === 'Publicado'
                  ...
                  >
                    <div
                      className={`w-1.5 h-1.5 rounded-full ${curso.estado === 'Publicado' ? 'bg-emerald-500' : 'bg-amber-500'}`}
                    />
                    {curso.estado}
                  </div>
      ...
      <ArchiveModal
        ...
        nuevoEstado={cursoEstadoSeleccionado?.estado === 'Publicado' ? 'Archivado' : 'Publicado'}
      />
      <DeleteModal
