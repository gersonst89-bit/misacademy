...
export default function RegisterForm() {
  ...
  return (
    <section className="flex items-center justify-center px-4 text-white overflow-hidden py-10 lg:py-12">
      ...
      >
        <motion.h2 variants={itemVariants} className="text-3xl md:text-5xl font-extrabold mb-4">
        ...
        >
          Únete a <span className="font-semibold">MIS ACADEMY</span> y comienza tu viaje hacia el
        ...
        >
          ...
          {['nombre', 'apellido'].map((field) => (
            <div key={field} className="flex items-center bg-white/10 rounded-lg px-4 py-3 mb-4">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            disabled={loading}
            className={`w-full py-3 rounded-lg flex items-center justify-center gap-2 transition-all font-bold text-white ${
              loading
                ? 'opacity-60 cursor-not-allowed bg-sky-500/50'
                : 'bg-gradient-to-r from-sky-500 to-indigo-500 hover:from-sky-400 hover:to-indigo-400 shadow-lg hover:shadow-sky-500/20'
            }`}
          >
            {loading ? (
              'Enviando...'
