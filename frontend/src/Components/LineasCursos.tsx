...
const styleMap: Record<string, any> = {
  ia: {
    icon: (
      <BookOpen className="w-8 h-8 text-green-400 group-hover:scale-110 transition-transform duration-300" />
    ),
  ...
  educacion: {
    icon: (
      <GraduationCap className="w-8 h-8 text-amber-400 group-hover:scale-110 transition-transform duration-300" />
    ),
  ...
  negocios: {
    icon: (
      <TrendingUp className="w-8 h-8 text-rose-400 group-hover:scale-110 transition-transform duration-300" />
    ),
...
const defaultStyle = {
  icon: (
    <BookOpen className="w-8 h-8 text-sky-400 group-hover:scale-110 transition-transform duration-300" />
  ),
    ...
    <section className="pt-24 pb-12 text-white relative overflow-hidden bg-[#03070c] lineas-cursos-rustica">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="text-center mb-16">
          ...
          >
            Explora nuestras líneas académicas y encuentra rutas y cursos diseñados para impulsar tu
          ...
          {lineas.map((linea: LineaData, index: number) => {
            ...
            return (
              <motion.div
                ...
                key={linea.id_linea_academica || `linea-${index}`}
              ...
              >
                ...
                >
                  <div className="relative mb-8">
                    <div
                      className={`absolute inset-0 blur-2xl opacity-20 group-hover:opacity-40 transition-opacity ${style.underlineColor}`}
                    />
                    <div
                      className={`w-16 h-16 rounded-2xl flex items-center justify-center bg-white/5 border border-white/10 relative z-10`}
                    >
                      {style.icon}
                    </div>
                  </div>
