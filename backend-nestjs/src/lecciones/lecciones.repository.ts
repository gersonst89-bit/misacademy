...
export class LeccionesRepository {
  ...
  async findById(id: number) {
    return this.leccionRepo.findOne({
      where: { id_leccion: id },
  ...
  async update(id: number, data: Partial<Leccion> & { duracion?: number }) {
    ...
    await this.leccionRepo.update({ id_leccion: id }, updateData);
  ...
  async findAll(page = 1, perPage = 20, filters: LeccionFilters = {}) {
    ...
    if (filters.query) {
      qb.andWhere('(l.titulo LIKE :q OR l.descripcion LIKE :q)', {
        q: `%${filters.query}%`,
      });
    }
  ...
  async completar(
    leccionId: number,
    userId: number,
    data: Partial<ProgresoEstudiante>,
  ) {
    ...
    const inscripcion = await this.inscripcionRepo.findOne({
      where: { id_usuario: userId, id_curso: leccion.modulo.id_curso },
    });
    ...
    if (!progreso) {
      ...
      if (data.ultimo_segundo_visto)
        progreso.ultimo_segundo_visto = data.ultimo_segundo_visto;
      if (data.segmentos_vistos)
        progreso.segmentos_vistos = data.segmentos_vistos;
  ...
  async navegacion(leccionId: number) {
    ...
    if (!leccion) return null;
    const lecciones = await this.leccionRepo.find({
      where: { id_modulo: leccion.id_modulo },
      order: { orden: 'ASC' },
    });
  ...
  async deleteComentario(comentarioId: number) {
    await this.comentarioRepo.delete({ id_comentario: comentarioId });
  }
  ...
  async heartbeat(
    ...
    const leccion = await this.leccionRepo.findOne({
      where: { id_leccion: data.id_leccion },
      relations: ['modulo'],
    });
    if (!leccion) return null;
    const inscripcion = await this.inscripcionRepo.findOne({
    ...
    if (!progreso) {
      progreso = this.progresoRepo.create({
        ...
        estado: 'En Progreso',
        ultima_actividad: new Date(),
        ...(data.primera_visualizacion === undefined
          ? { primera_visualizacion: new Date() }
          : {}),
      });
    }
    if (data.ultimo_segundo_visto !== undefined)
      progreso.ultimo_segundo_visto = data.ultimo_segundo_visto;
    if (data.segmentos_vistos !== undefined)
      progreso.segmentos_vistos = data.segmentos_vistos;
    if (data.duracion_video !== undefined)
      progreso.duracion_video = data.duracion_video;
    if (data.porcentaje_completado !== undefined)
      progreso.porcentaje_completado = data.porcentaje_completado;
    progreso.ultima_actividad = new Date();
    return this.progresoRepo.save(progreso);
  }
}
