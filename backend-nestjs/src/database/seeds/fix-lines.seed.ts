...
export async function seedFixLines(dataSource: DataSource): Promise<void> {
  ...
  const vinculacionCursosRutas = [
    ...
    },
    {
      ...
      cursosNames: [
        'SEO y Posicionamiento para Desarrolladores',
        'Marketing Digital y Marca Personal para Tech',
      ],
    },
    {
      rutaId: 14, // Finanzas (MIS BUSINESS)
      cursosNames: [
        'Finanzas para Freelancers Tecnológicos',
        'Inglés Técnico para Profesionales IT',
      ],
    },
    {
  ...
  for (const vinc of vinculacionCursosRutas) {
    ...
    if (ruta) {
      ...
      for (const nombre of vinc.cursosNames) {
        ...
        if (
          curso &&
          !cursosExistentes.some((ce) => ce.id_curso === curso.id_curso)
        ) {
      ...
      console.log(
        `  🔗 Ruta "${ruta.nombre}" vinculada. Total cursos: ${ruta.cursos.length}`,
      );
    }
  }
