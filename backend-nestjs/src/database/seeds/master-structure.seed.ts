...
export async function seedMasterStructure(
  ...
  console.log(
    '🏗️  Iniciando RE-ESTRUCTURACIÓN MAESTRA (3 Rutas/Línea, 3 Cursos/Ruta)...',
  );
  ...
  const lineasData = [
    {
      ...
      nombre: 'MIS TEACHER',
      slug: 'mis-teacher',
      desc: 'Recursos y herramientas para mejorar la enseñanza',
    },
    {
      id: 2,
      nombre: 'MIS IA',
      slug: 'mis-ia',
      desc: 'Aprende inteligencia artificial y sus aplicaciones',
    },
    {
      id: 3,
      nombre: 'MIS BUSINESS',
      slug: 'mis-business',
      desc: 'Estrategias y gestión para negocios exitosos.',
    },
    {
      id: 4,
      nombre: 'MIS DEV',
      slug: 'mis-dev',
      desc: 'Formación integral en desarrollo de software y programación moderna',
    },
  ];

  for (const l of lineasData) {
    let linea = await lineaRepo.findOne({
      where: { id_linea_academica: l.id },
    });
    if (!linea) {
  ...
  for (const item of estructura) {
    const rutasAntiguas = await rutaRepo.find({
      where: { id_linea_academica: item.lineaId },
      relations: ['cursos'],
    });
  ...
  for (const item of estructura) {
    ...
    for (const rData of item.rutas) {
      let ruta = await rutaRepo.findOne({
        where: { nombre: rData.nombre, id_linea_academica: item.lineaId },
        relations: ['cursos'],
      });
      ...
      for (const cNombre of rData.cursos) {
        ...
        if (!curso) {
          ...
          palabras.forEach((p, i) => {
            if (i === 0)
              query.where('c.nombre LIKE :p' + i, { ['p' + i]: `%${p}%` });
            else
              query.andWhere('c.nombre LIKE :p' + i, { ['p' + i]: `%${p}%` });
          });
      ...
      await rutaRepo.save(ruta);
      console.log(
        `    🔗 Ruta "${rData.nombre}" sincronizada con ${cursosParaVincular.length} cursos.`,
      );
    }
  }
