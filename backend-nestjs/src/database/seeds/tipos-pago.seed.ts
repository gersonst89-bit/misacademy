...
export async function seedTiposPago(dataSource: DataSource): Promise<void> {
    ...
    try {
        ...
        if (columns.length === 0) {
            ...
            await dataSource.query('UPDATE materiales m JOIN modulos mod ON m.id_modulo = mod.id_modulo SET m.id_curso = mod.id_curso WHERE m.id_curso IS NULL');
        }
        
        const colEstado = await dataSource.query('SHOW COLUMNS FROM materiales LIKE "estado"');
        if (colEstado.length === 0) {
            await dataSource.query('ALTER TABLE materiales ADD COLUMN estado VARCHAR(20) DEFAULT "Activo"');
        }
    ...
    const tipos = [
        { nombre: 'Yape', descripcion: 'Pago rápido con QR o número', activo: true },
        { nombre: 'Plin', descripcion: 'Pago rápido con QR o número', activo: true },
        { nombre: 'Transferencia Bancaria', descripcion: 'BCP, BBVA, Interbank', activo: true },
        { nombre: 'Tarjeta de Crédito/Débito', descripcion: 'Próximamente (Pasarela)', activo: false },
    ];
