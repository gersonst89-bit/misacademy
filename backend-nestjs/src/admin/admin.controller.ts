...
export class AdminUsuariosController {
  ...
  @Get() findAll(@Query() q: any) {
    return this.svc.findAllUsuarios(q);
  }
  @Post() create(@Body() dto: CreateUsuarioDto) {
    return this.svc.createUsuario(dto);
  }
  @Put(':id') update(@Param('id') id: number, @Body() dto: UpdateUsuarioDto) {
    return this.svc.updateUsuario(id, dto);
  }
  @Delete(':id') deactivate(@Param('id') id: number) {
    return this.svc.deactivateUsuario(id);
  }
  @Delete(':id/force') forceDelete(
    @Param('id') id: number,
    @CurrentUser() u: Usuario,
  ) {
    return this.svc.forceDeleteUsuario(id, u.id_usuario);
  }
}
...
export class AdminMaterialesController {
  constructor(private readonly svc: AdminService) {}
  @Get() findAll(@Query('page') p?: number) {
    return this.svc.findAllMateriales(p);
  }
