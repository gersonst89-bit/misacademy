...
export class EstadisticasRepository {
  ...
  async getDashboard() {
    ...
    const ingresoTotal = await this.pagoRepo
      ...
      .where('p.estado = :e', { e: 'Completado' })
      .select('SUM(p.monto_total)', 'total')
      .getRawOne();
    const inscripcionesRecientes = await this.inscripcionRepo.find({
  ...
  async getRetencionMensual() {
    ...
    const activos = await this.usuarioRepo.count({
      where: { estado: 'Activo' },
    });
