...
export class ChatbotService implements OnModuleInit {
    ...
    private async trainBot() {
        ...
        this.manager.addAnswer('es', 'saludo.hola', `🏛️ *ASISTENTE VIRTUAL - MIS ACADEMY* 🎓
...
7️⃣ Certificados y Metodología`);
        ...
        this.manager.addAnswer('es', 'saludo.despedida', '¡Hasta pronto! Recuerda que aquí estoy si necesitas más ayuda. ¡Éxitos en tu aprendizaje!');
        ...
        this.manager.addDocument('es', 'tienen cursos de programacion', 'info.cursos');
        ...
        this.manager.addDocument('es', 'muestrame los cursos disponibles', 'info.cursos');
        ...
        this.manager.addDocument('es', 'precio del curso de fundamentos', 'info.precio');
        ...
        this.manager.addDocument('es', 'que areas de estudio tienen', 'info.lineas');
        this.manager.addDocument('es', 'cuales son sus especialidades', 'info.lineas');
        ...
        this.manager.addDocument('es', 'que rutas de aprendizaje tienen', 'info.rutas');
        this.manager.addDocument('es', 'que es mis teacher o mis dev', 'info.rutas');
        ...
        this.manager.addDocument('es', 'cual es la ruta mas economica', 'info.rutas');
        ...
        this.manager.addDocument('es', 'al terminar dan diploma', 'info.certificados');
        this.manager.addDocument('es', 'los cursos estan avalados', 'info.certificados');
        this.manager.addDocument('es', 'tiene certificacion', 'info.certificados');
        this.manager.addDocument('es', 'los cursos tienen certificado', 'info.certificados');
        this.manager.addDocument('es', 'dan certificado con los cursos', 'info.certificados');
        this.manager.addDocument('es', 'cursos con certificado', 'info.certificados');
        this.manager.addDocument('es', 'tienen algun certificado', 'info.certificados');
        this.manager.addAnswer('es', 'info.certificados', '¡Sí! Todos nuestros cursos y rutas incluyen un certificado digital de finalización que puedes agregar a tu LinkedIn y CV para impulsar tu carrera profesional.');

        // 6. Metodología (En vivo o grabado)
        this.manager.addDocument('es', 'las clases son en vivo', 'info.metodologia');
        this.manager.addDocument('es', 'son clases grabadas', 'info.metodologia');
        this.manager.addDocument('es', 'como es la metodologia', 'info.metodologia');
        this.manager.addDocument('es', 'tengo un horario fijo', 'info.metodologia');
        this.manager.addDocument('es', 'cuanto duran los cursos', 'info.metodologia');
        this.manager.addAnswer('es', 'info.metodologia', 'Nuestra plataforma es 100% online y a tu propio ritmo. Las lecciones están pregrabadas en alta calidad para que estudies cuando y donde quieras, sin horarios fijos.');
        ...
        this.manager.addAnswer('es', 'info.pagos', 'Aceptamos pagos mediante Yape, Plín y Transferencia Bancaria (BCP, BBVA, Interbank). Muy pronto habilitaremos también el pago directo con tarjeta de crédito y débito.');

        // 8. Soporte Humano / Problemas
        this.manager.addDocument('es', 'quiero hablar con un humano', 'ayuda.soporte');
        ...
        this.manager.addAnswer('es', 'ayuda.soporte', 'Si necesitas ayuda personalizada, puedes escribirnos directamente a nuestro correo oficial de soporte o al WhatsApp de la academia. ¡Un asesor humano te atenderá con gusto!');

        // 8b. Información General de la Plataforma
        this.manager.addDocument('es', 'informacion sobre la plataforma', 'info.plataforma');
        ...
        this.manager.addDocument('es', 'de que trata la plataforma', 'info.plataforma');
        ...
        this.manager.addAnswer('es', 'bot.identidad', 'Soy el asistente virtual inteligente de MIS ACADEMY, creado específicamente para ayudarte a resolver tus dudas sobre nuestra plataforma académica. 🤖🎓');
        ...
        this.manager.addAnswer('es', 'emocion.gracias', '¡Con mucho gusto! Estoy aquí para ayudarte siempre que lo necesites. Si tienes más preguntas sobre cursos, rutas o precios, ¡no dudes en escribirme! 😊');
        ...
        this.manager.addAnswer('es', 'info.contacto', 'Puedes contactarnos por los siguientes canales:\n📧 Email: contacto@misacademy.edu.pe\n📱 WhatsApp: +51 999 999 999\n🌐 Web: www.misacademy.edu.pe\n¡Estamos para servirte!');
        ...
        this.manager.addAnswer('es', 'emocion.objecion', 'Entendemos que es una decisión importante. Recuerda que invertir en educación es la mejor inversión que puedes hacer. Nuestros cursos incluyen certificado, soporte y acceso de por vida. Además, tenemos cursos desde S/ 99.00. ¿Te gustaría que te muestre las opciones más accesibles?');
        ...
        this.manager.addAnswer('es', 'emocion.queja', 'Lamento no haber podido ayudarte como esperabas. Estoy aprendiendo cada día para ser mejor. Si prefieres hablar con una persona real, puedes contactar a nuestro equipo de soporte. ¡Queremos asegurarnos de resolver tu duda! 🙏');
...
7️⃣ Certificados y Metodología`;
            ...
            if (lowerMsg === '1') {
                ...
                const courses = await this.cursoRepo.find({ where: { estado: 'Publicado' } });
                const courseList = courses.slice(0, 8).map(c => `📚 *${c.nombre}*`).join('\n');
                const sufijo = courses.length > 8 ? `\n\n_...y ${courses.length - 8} cursos más en nuestra web._` : '';
                ...
                const courses = await this.cursoRepo.find({ where: { estado: 'Publicado' } });
                const cheapest = [...courses].sort((a, b) => Number(a.precio || 0) - Number(b.precio || 0))[0];
                ...
                // Cursos Gratis
                const esGratis = lowerMsg.includes('gratis') || lowerMsg.includes('gratuito') || lowerMsg.includes('free');
                if (esGratis) {
                    const courses = await this.cursoRepo.find({ where: { estado: 'Publicado' } });
                    const cheapest = [...courses].sort((a, b) => Number(a.precio || 0) - Number(b.precio || 0))[0];
