...
export class AuthRepository {
  ...
  async findByEmail(email: string): Promise<Usuario | null> {
    return this.usuarioRepo.findOne({
      where: { email },
      relations: ['rol'],
      select: ['id_usuario', 'nombre', 'apellido', 'email', 'password', 'id_rol', 'email_verificado', 'estado', 'imagen_perfil', 'biografia', 'telefono', 'dni', 'fecha_registro'],
    });
  }

  async findById(id: number): Promise<Usuario | null> {
    return this.usuarioRepo.findOne({ where: { id_usuario: id }, relations: ['rol'] });
  }
  ...
  async markEmailVerified(userId: number): Promise<void> {
    await this.usuarioRepo.update({ id_usuario: userId }, { email_verificado: true });
  }

  async createAuthLog(data: Partial<AuthenticationLog>): Promise<AuthenticationLog> {
  ...
  async updateLogout(userId: number): Promise<void> {
    const lastLogin = await this.authLogRepo.findOne({
      where: { authenticatable_id: userId, authenticatable_type: 'App\\Models\\Usuario', logout_at: undefined },
      order: { login_at: 'DESC' },
    });
    if (lastLogin) {
  ...
  async updateAvatar(userId: number, imageUrl: string): Promise<void> {
    await this.usuarioRepo.update({ id_usuario: userId }, { imagen_perfil: imageUrl });
  }
}
