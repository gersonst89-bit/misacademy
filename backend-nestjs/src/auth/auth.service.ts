...
export class AuthService {
  ...
  private get refreshSecret(): string {
    ...
    if (!secret) {
      throw new Error(
        'REFRESH_JWT_SECRET env variable is required but not set',
      );
    }
    return secret;
  }
  ...
  async register(dto: RegisterDto) {
    ...
    try {
      ...
      this.logger.log(
        `Email de verificación enviado a: ${verificationMail.to}`,
      );
    } catch (error) {
      this.logger.error(
        `Error enviando email a ${verificationMail.to}:`,
        error,
      );
    }
  ...
  async login(dto: LoginDto, ip: string, userAgent: string) {
    ...
    const passwordIsValid = await bcrypt.compare(dto.password, passwordHash);
  ...
  async forgotPassword(dto: ForgotPasswordDto) {
    ...
    if (user) {
      ...
      try {
        ...
        this.logger.log(`Email de reset enviado a: ${resetMail.to}`);
      } catch (error) {
        this.logger.error(`Error enviando email a ${resetMail.to}:`, error);
      }
    }
    return {
      message:
  ...
  async resetPassword(dto: ResetPasswordDto) {
    ...
    await this.authRepo.invalidateAllUserRefreshTokens(user.id_usuario);
  ...
  async changePassword(user: Usuario) {
    ...
    try {
      ...
      this.logger.error(
        `CHANGE PASSWORD -> ERROR: ${error?.message || String(error)}`,
      );
