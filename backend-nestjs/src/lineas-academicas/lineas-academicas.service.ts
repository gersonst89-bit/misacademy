...
export class LineasAcademicasService {
  ...
  async findAllRutasAdmin(q: QueryDto) {
    return this.repo.findAllRutasAdmin(q, q.page, q.per_page);
  }
  ...
  async findRutaBySlug(slug: string) {
    const r = await this.repo.findRutaBySlug(slug);
  ...
  async findLineasMenu() {
    return this.repo.findLineasMenu();
  }

  async findRutasMenu() {
    return this.repo.findRutasMenu();
  }
  async findLineaByCursoId(idCurso: number) {
    return this.repo.findLineaByCursoId(idCurso);
  }
}
